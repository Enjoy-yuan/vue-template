  ...
  <div>
    <el-dropdown @command="handleCommand" style="margin-left: 30px;margin-top: 10px;">
      ...
      <el-dropdown-menu slot="dropdown">
        <el-dropdown-item :command="item.key" v-for="(item, index) in optionsType" :key="index">{{ item.text }}</el-dropdown-item>
      </el-dropdown-menu>
    </el-dropdown>
...
export default {
  ...
  methods: {
    ...
    resizeEcharts() {
      // 图例大小的适配
      const titleFontSize = (this.$refs.echart.offsetWidth / 100) * 1.5
      let adapterOption = {}
      if (titleFontSize > 12) {
        adapterOption = {
          legend: {
