...
export default {
  ...
  beforeDestroy() {
    this.oContent.removeEventListener('scroll', this.handleScroll)
    if (this.interval) {
  ...
  methods: {
    handleScroll() {
      this.visible = this.oContent.scrollTop > this.visibilityHeight
    },
    backToTop() {
      if (this.isMoving) return
      const start = this.oContent.scrollTop
      let i = 0
      this.isMoving = true
